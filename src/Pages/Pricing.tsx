...
const Pricing = () => {
  ...
  return (
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      {/* Grid Layout */}
       <h1 className="text-4xl md:text-6xl !leading-[1.4] font-bold mb-4 text-center md:mb-8">
                           Pricing
                        </h1>
        ...
        {plans.map((plan, index) => (
          ...
          >
            ...
            <div className="mb-4">
              ...
              {/* <p className={`text-sm leading-relaxed ${plan.isPopular ? '' : ''}`}>
                {plan.description}
              </p> */}
            </div>
            ...
            {/* Subscribe Button */}
            <button
              className="w-full py-3 px-4 rounded-full font-semibold text-white bg-bluecustom hover:bg-bluecustomhover transition-all duration-200 mb-8"
            >
              {plan.buttonText}
            </button>
              ...
              {plan.features.map((feature, featureIndex) => (
                <div key={featureIndex} className="flex items-start gap-3">
                  <div className={`flex-shrink-0 w-5 h-5 rounded-full flex items-center justify-center mt-0.5 ${
                    ...
                    <Check className={`w-3 h-3 ${plan.isPopular ? 'text-yellow-400' : 'text-gray-600'}`} />
                  </div>
                  <span 
